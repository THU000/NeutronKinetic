function neutron = sineCalculateNeutronWithRhoArray(T, dt, rhoArray, lambda, beta, age)
...
for n = 2:(N + 1)
    rho = rhoArray(n)*sin(n*dt*pi/5);
    Lambda = age * (1 - rho);
    F1 = dt * rhoArray(n)*(sin(n*dt*pi/5)+sin((n-1)*dt*pi/5)) / 2*Lambda;
    F2 = -0.5 * dt * F1;
