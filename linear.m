%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   线性反应性，改变反应性大小
%   配合函数文件 linearCalculateNeutronWithRhoArray.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:4
    ...
    neutron_change(i,:)=linearCalculateNeutronWithRhoArray(T,dt,rho,lambda,beta,age);
end

figure; % 创建新的图形窗口

% 绘制四条中子数量变化曲线，使用对数纵坐标
semilogy(t, neutron_change(1, :), 'LineWidth', 1.5, 'DisplayName',['rho = ', num2str(rho_0(1)),'t']);
hold on;
for i = 2:4
    semilogy(t, neutron_change(i, :), 'LineWidth', 1.5, 'DisplayName',['rho = ', num2str(rho_0(i)),'t']);
end

% 设置坐标轴标签和标题
xlabel('时间（s）');
ylabel('中子密度（对数坐标）');
title('不同线性增加正反应性下中子密度随时间变化曲线');
% 设置横轴范围（可根据需求调整）
xlim([0, 100]); 
